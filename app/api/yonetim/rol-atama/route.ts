...
export default function RolAtamaPage() {
  ...
  const [degistirenPersonelId, setDegistirenPersonelId] = useState("")
  const [degistirenAd, setDegistirenAd] = useState("")
  ...
  async function verileriYukle() {
    ...
    if (session?.user) {
      const { data: aktifPersonel } = await supabase
        .from("personeller")
        .select("id, ad, soyad")
        .or(`auth_id.eq.${session.user.id},kullanici_id.eq.${session.user.id},email.eq.${session.user.email}`)
        .maybeSingle()

      if (aktifPersonel) {
        setDegistirenPersonelId(aktifPersonel.id)
        setDegistirenAd(`${aktifPersonel.ad || ""} ${aktifPersonel.soyad || ""}`.trim())
      }
    }
  ...
  async function kaydet() {
    ...
    const response = await fetch("/api/yonetim/rol-atama", {
      ...
      body: JSON.stringify({
        ...
        yeni_rol: seciliRol,
        degistiren_personel_id: degistirenPersonelId || null,
        degistiren_ad: degistirenAd || null,
    ...
    setMesaj({
      tip: "basari",
      metin: `${adSoyad(seciliPersonel)} rolü "${seciliRol}" olarak güncellendi. Değiştiren: ${degistirenAd || "Bilinmiyor"}`,
    })
              ...
              {seciliPersonel && (
                <div className="mt-6 space-y-5">
                  ...
                  <div className="rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm font-bold text-amber-900">
                    Rol değişikliği personelin operasyonel kimliğini belirler. Modül yetkileri otomatik verilmez; Yetki Yönetimi ekranından manuel yönetilir.
                  </div>
                </div>
              )}
            </div>
