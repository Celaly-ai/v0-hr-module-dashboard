      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
            Tamamlanmış servis fişleri tekil veya Excel ile anket havuzuna alınır. Bu sayfa büro ve operasyon ekibi içindir.
          </p>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <Card className="p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-black">Excel Toplu Yükleme</h2>
          </div>

          <p className="text-xs text-muted-foreground">
            Excel’de ürün grubu ve model serbest yazı olabilir. Sistem kayıtları anket havuzuna alır.
          </p>
          ...
          <input
            ...
            className="mt-4 w-full rounded-lg border border-border p-2 text-xs"
          />
        ...
        <Card className="p-5 xl:col-span-2">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-black">Tekil Tamamlanmış İş Girişi</h2>
          </div>

          <div className="grid gap-3 md:grid-cols-2">
            <input className="rounded-lg border p-2 text-sm" placeholder="Müşteri adı" value={isForm.musteri_adi} onChange={(e) => formGuncelle("musteri_adi", e.target.value)} />
            <input className="rounded-lg border p-2 text-sm" placeholder="Fiş numarası" value={isForm.fis_numarasi} onChange={(e) => formGuncelle("fis_numarasi", e.target.value)} />
            <input className="rounded-lg border p-2 text-sm" placeholder="Telefon" value={isForm.telefon} onChange={(e) => formGuncelle("telefon", e.target.value)} />
            <input className="rounded-lg border p-2 text-sm" placeholder="Başvuru nedeni" value={isForm.basvuru_nedeni} onChange={(e) => formGuncelle("basvuru_nedeni", e.target.value)} />

            <select className="rounded-lg border p-2 text-sm" value={isForm.ilce} onChange={(e) => formGuncelle("ilce", e.target.value)}>
              <option value="">İlçe seç</option>
              {ilceler.map((item) => <option key={item.id} value={adAl(item)}>{adAl(item)}</option>)}
            </select>

            <select className="rounded-lg border p-2 text-sm" value={isForm.mahalle} onChange={(e) => formGuncelle("mahalle", e.target.value)}>
              <option value="">Mahalle seç</option>
              {filtreliMahalleler.map((item) => <option key={item.id} value={adAl(item)}>{adAl(item)}</option>)}
            </select>

            <select
              className="rounded-lg border p-2 text-sm"
              value={isForm.teknisyen_kodu}
              onChange={(e) => {
                const secili = teknisyenler.find((item) => metin(item.personel_kodu) === e.target.value)
                setIsForm((onceki) => ({
                  ...onceki,
                  teknisyen_kodu: e.target.value,
                  teknisyen_adi: secili ? personelAdi(secili) : "",
                }))
              }}
            >
              <option value="">Teknisyen seç</option>
              {teknisyenler.map((item) => (
                <option key={item.id || item.personel_kodu} value={metin(item.personel_kodu)}>
                  {metin(item.personel_kodu)} - {personelAdi(item)}
                </option>
              ))}
            </select>

            <select className="rounded-lg border p-2 text-sm" value={isForm.marka} onChange={(e) => formGuncelle("marka", e.target.value)}>
              <option value="">Marka seç</option>
              {markalar.map((item) => <option key={item.id} value={adAl(item)}>{adAl(item)}</option>)}
            </select>

            <select className="rounded-lg border p-2 text-sm" value={isForm.urun_grubu} onChange={(e) => formGuncelle("urun_grubu", e.target.value)}>
              <option value="">Ürün grubu seç</option>
              {urunGruplari.map((item) => <option key={item.id} value={adAl(item)}>{adAl(item)}</option>)}
            </select>

            <select className="rounded-lg border p-2 text-sm" value={isForm.model} onChange={(e) => formGuncelle("model", e.target.value)}>
              <option value="">Model seç</option>
              {filtreliModeller.map((item) => <option key={item.id} value={adAl(item)}>{adAl(item)}</option>)}
              <option value="__serbest__">Listede yok / Serbest yaz</option>
            </select>

            {isForm.model === "__serbest__" && (
              <input className="rounded-lg border p-2 text-sm" placeholder="Model serbest yazı" value={isForm.model_serbest} onChange={(e) => formGuncelle("model_serbest", e.target.value)} />
            )}

            <select className="rounded-lg border p-2 text-sm" value={isForm.yapilan_hizmet_kodu} onChange={(e) => formGuncelle("yapilan_hizmet_kodu", e.target.value)}>
              <option value="">Yapılan hizmet kodu seç</option>
              {hizmetTipleri.map((item) => <option key={item.id} value={adAl(item)}>{adAl(item)}</option>)}
            </select>
          </div>

          <textarea className="mt-3 min-h-20 w-full rounded-lg border p-2 text-sm" placeholder="Kullanılan malzeme açıklaması" value={isForm.kullanilan_malzeme_aciklama} onChange={(e) => formGuncelle("kullanilan_malzeme_aciklama", e.target.value)} />
          <textarea className="mt-3 min-h-20 w-full rounded-lg border p-2 text-sm" placeholder="Teknisyen notu / yapılan hizmet açıklaması" value={isForm.teknisyen_notu} onChange={(e) => formGuncelle("teknisyen_notu", e.target.value)} />

          <button
            type="button"
            onClick={isEkle}
            disabled={isKaydediliyor}
            className="mt-4 inline-flex min-h-10 items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-black text-primary-foreground disabled:opacity-60"
          >
            {isKaydediliyor ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
            İşi Anket Havuzuna Ekle
          </button>
        </Card>
      </div>
