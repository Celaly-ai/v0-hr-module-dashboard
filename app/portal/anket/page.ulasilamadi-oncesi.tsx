...
export default function AnketPage() {
  ...
  async function yoneticiyeIlet(anketId: number) {
    ...
    const { data, error } = await supabase.rpc("ai_anket_operasyon_aksiyon_olustur", {
      p_anket_id: anketId,
      p_not: not,
    })

    if (error || data?.success === false) {
      setHata(error?.message || data?.error || "Operasyon aksiyonu oluşturulamadı.")
      setYoneticiyeIletiliyorId(null)
      return
    }

    if (data?.created === false) {
      setBilgi(data?.message || "Bu anket için operasyon aksiyonu zaten mevcut veya gerekli değil.")
    } else {
      setBilgi(`Operasyon aksiyonu oluşturuldu: ${data?.aksiyon_kodu || ""}`)
    }
